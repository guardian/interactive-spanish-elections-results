...
/*let al = csv(almeria,{"delimiter": ";","rtrim": true});
...
al.map(field => {
	...
	municipality.push({
			'comunidad-code':field[0],
			'provincia-code':field[1],
			'fixed-value-0':field[2],
			'municipality-code':field[3],
			'district-code':field[4],
			'municipality-name':field[5],
			'fixed-value-1':field[6],
			'fixed-value-2':field[7],
			'poll-boxes':field[8],
			'census-total':field[9],
			'census-counted':field[10],
			'census-counted-percentage':field[11],
			'voters':field[12],
			'voters-percentage':field[13],
			'abstention':field[14],
			'abstention-percentage':field[15],
			'blank-votes':field[16],
			'blank-votes-percentage':field[17],
			'null-votes':field[18],
			'null-votes-percentage':field[19],
			'results':[]
	})
	...
	for (let i = 20; i < 340; i++) {
		...
		if(i % 4 == 0){
			municipality[0].results.push({
				'party-code':i,//field[i],
				'party-acronym':i + 1,//field[i + 1],
				'party-votes':i + 2,//field[i + 2],
				'party-votes-percentage':i + 3//field[i + 3]
			})
		}
	}
